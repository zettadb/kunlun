...
static AttrNumber
get_attrnum(Relation rel, const char *colname)
{
	for (int i = 0; i < rel->rd_att->natts; ++i)
	{
		if (strcasecmp(NameStr(rel->rd_att->attrs[i].attname), colname) == 0)
			return i+1;
	}
...
void log_alter_table(PlannedStmt *pstmt, AlterTableStmt *stmt, const char *query)
{
	...
	{
		/**
		 * Rewrite shard option of identity sequence :
		 *   alter table t3 alter column a add generated by default as identity(sequence name s3);
		 * =>
		 *   alter table t3 alter column a add generated by default as identity(sequence name s3 shard=?);
		 */
		List *subcmdlst = NIL;
		ListCell *lc;
		AttrNumber attrno;
		...
		foreach (lc, stmt->cmds)
		{
			...
			if (subcmd->subtype == AT_AddIdentity)
			{
		...
		foreach (lc, subcmdlst)
		{
			AlterTableCmd *subcmd = lfirst_node(AlterTableCmd, lc);
			Constraint *constraint = (Constraint *)subcmd->def;
			/* Get attrno of column */
			attrno = get_attrnum(rel, subcmd->name);
			Assert(attrno != InvalidAttrNumber);

			/* The corresponding sequence */
			seqid = getOwnedSequence(RelationGetRelid(rel), attrno);
			Assert(seqid != InvalidOid);

			{
				...
				DefElem *name, *shard;
				name = shard = NULL;
				foreach (elem, constraint->options)
				{
					const char *opt_name = lfirst_node(DefElem, elem)->defname;
					if (!name && strcasecmp(opt_name, "sequence_name") == 0)
						name = lfirst_node(DefElem, elem);
					if (!shard && strcasecmp(opt_name, "shard") == 0)
						shard = lfirst_node(DefElem, elem);
				}
				/* add extra option*/
				Relation seqrel = relation_open(seqid, NoLock);
				 ...
				 *  ADD GENERATED BY DEFAULT AS IDENTITY [ ( sequence_options ) ]
				 */
				int location;
				bool add_brackets;
				char c;
				...
				StringInfoData buff;
				initStringInfo(&buff);
				appendStringInfo(&buff, "%.*s %c%s%c %s", location, query,
						add_brackets ? '(' : ' ',
						extra.data,
						add_brackets ? ')' : ' ',
						query + location);
