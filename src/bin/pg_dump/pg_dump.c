...
dumpSequence(Archive *fout, TableInfo *tbinfo)
{
	...
	if (tbinfo->is_identity_sequence)
	{
		...
		char attidentity = owning_tab->attidentity[tbinfo->owning_col - 1];
		...
		case ATTRIBUTE_IDENTITY_ALWAYS:
			appendPQExpBuffer(query, "GENERATED ALWAYS AS IDENTITY");
			break;
		case ATTRIBUTE_IDENTITY_BY_DEFAULT:
			appendPQExpBuffer(query, "GENERATED BY DEFAULT AS IDENTITY");
			break;
		case ATTRIBUTE_IDENTITY_AUTO_INCREMENT:
			appendPQExpBuffer(query, "AUTO_INCREMENT");
			break;
		}
		appendPQExpBuffer(query, " (\n    SEQUENCE NAME %s\n",
				  fmtQualifiedDumpable(tbinfo));
