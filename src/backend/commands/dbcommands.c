...
createdb(ParseState *pstate, const CreatedbStmt *stmt)
{
	 ...
	 *
	 * TODO: map pg 'create database' stmt's encoding, collation and locale
	 * to mysql 'create database' stmt's charset, collation and locale
	 * correctly! all other options here can be ignored.
	 *
	 * The 1st mysql db to create uses the 'public' schema name.
	 * */
	...
	/*
	  dzw: only allow UTF8 as collation and ctype and encoding, we always do
	  SET NAMES 'UTF8' in connection with storage shards.
	*/
	if (encoding != PG_UTF8 ||
		(strcasestr(dbcollate, "UTF-8") == NULL &&
		 strcasestr(dbcollate, "UTF8") == NULL) ||
		(strcasestr(dbctype, "UTF-8") == NULL &&
		 strcasestr(dbctype, "UTF8") == NULL))
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg("cannot set LC_COLLATE/LC_CTYPE/ENCODING to (encoding: %d, dbcollate: %s, dbctype: %s) in Kunlun, we require UTF8 encoding and collation throughout Kunlun clusters.",
	...
	{
		/*
		  dzw: we have banned creating tablespaces so if user specifies a
		  tablespace here, it's a harmless default one and such stmts are often
		  generated by tools, and we should not ban it.
		*/
...
dropdb(const char *dbname, bool missing_ok)
{
	...
	if (strcasecmp(dbname, "postgres") == 0)
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg("Can not drop 'postgres' database in Kunlun database cluster.")));
	/*
...
/*
 * get_database_name - given a database OID, look up the name, and
 * store it into 'str'.
 *
 * Returns >0 if db name is found, it's the db name string length, -1 otherwise.
 */
int get_database_name2(Oid dbid, StringInfo str)
{
	HeapTuple	dbtuple;
	int ret = -1;
	...
	if (HeapTupleIsValid(dbtuple))
	{
		ret = appendStringInfoString(str, NameStr(((Form_pg_database) GETSTRUCT(dbtuple))->datname));
		ReleaseSysCache(dbtuple);
	}
...
int get_database_name3(Oid dbid, NameData *nm)
{
	HeapTuple	dbtuple;
	int ret = -1;
	...
	if (HeapTupleIsValid(dbtuple))
	{
		char *end = stpncpy(nm->data, NameStr(((Form_pg_database) GETSTRUCT(dbtuple))->datname), sizeof(*nm) - 1);
		*end = '\0';
		ret = end - nm->data;
		ReleaseSysCache(dbtuple);
	}
